...
#include <trace/events/cpufreq_interactive2.h>
...
struct cpufreq_interactive2_cpuinfo {
...
static DEFINE_PER_CPU(struct cpufreq_interactive2_cpuinfo, cpuinfo);
...
struct cpufreq_interactive2_inputopen {
	struct input_handle *handle;
	struct work_struct inputopen_work;
};

static struct cpufreq_interactive2_inputopen inputopen;
...
static int cpufreq_governor_interactive2(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_INTERACTIVE2
static
#endif
struct cpufreq_governor cpufreq_gov_interactive2 = {
	.name = "interactive2",
	.governor = cpufreq_governor_interactive2,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static void cpufreq_interactive2_timer(unsigned long data)
{
	...
	u64 time_in_idle;
	u64 idle_exit_time;
	struct cpufreq_interactive2_cpuinfo *pcpu =
	...
	if (cpu_load >= go_hispeed_load || boost_val) {
		if (pcpu->target_freq <= pcpu->policy->min) {
			...
			if (pcpu->target_freq == hispeed_freq &&
				...
				trace_cpufreq_interactive2_notyet(data, cpu_load,
								 pcpu->target_freq,
								 new_freq);
				goto rearm;
			}
	...
	if (pcpu->target_freq == new_freq) {
		trace_cpufreq_interactive2_already(data, cpu_load,
						  pcpu->target_freq, new_freq);
		goto rearm_if_notmax;
	}

	trace_cpufreq_interactive2_target(data, cpu_load, pcpu->target_freq,
					 new_freq);
...
static void cpufreq_interactive2_tune(struct work_struct *work)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	for_each_cpu(cpu, &tmp_mask) {
		...
		for_each_cpu(j, pcpu->policy->cpus) {
			struct cpufreq_interactive2_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
...
static void cpufreq_interactive2_idle_start(void)
{
	struct cpufreq_interactive2_cpuinfo *pcpu =
...
static void cpufreq_interactive2_idle_end(void)
{
	struct cpufreq_interactive2_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
...
static int cpufreq_interactive2_up_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			if (max_freq != pcpu->policy->cur)
				__cpufreq_driver_target(pcpu->policy,
							...
							CPUFREQ_RELATION_H);
			mutex_unlock(&set_speed_lock);
			trace_cpufreq_interactive2_up(cpu, pcpu->target_freq,
						     pcpu->policy->cur);
		}
	}
...
static void cpufreq_interactive2_freq_down(struct work_struct *work)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	for_each_cpu(cpu, &tmp_mask) {
		...
		mutex_unlock(&set_speed_lock);
		trace_cpufreq_interactive2_down(cpu, pcpu->target_freq,
					       pcpu->policy->cur);
	}
}

static void cpufreq_interactive2_boost(void)
{
	int i;
	int anyboost = 0;
	unsigned long flags;
...
static void cpufreq_interactive2_input_event(struct input_handle *handle,
					    unsigned int type,
					    unsigned int code, int value)
{
	if (input_boost_val && type == EV_SYN && code == SYN_REPORT) {
		trace_cpufreq_interactive2_boost("input");
		cpufreq_interactive2_boost();
	}
}

static void cpufreq_interactive2_input_open(struct work_struct *w)
{
	struct cpufreq_interactive2_inputopen *io =
		container_of(w, struct cpufreq_interactive2_inputopen,
			     inputopen_work);
	int error;
...
static int cpufreq_interactive2_input_connect(struct input_handler *handler,
					     ...
					     const struct input_device_id *id)
{
	...
	handle->name = "cpufreq_interactive2";
...
static void cpufreq_interactive2_input_disconnect(struct input_handle *handle)
{
...
static const struct input_device_id cpufreq_interactive2_ids[] = {
	{
...
static struct input_handler cpufreq_interactive2_input_handler = {
	.event          = cpufreq_interactive2_input_event,
	.connect        = cpufreq_interactive2_input_connect,
	.disconnect     = cpufreq_interactive2_input_disconnect,
	.name           = "cpufreq_interactive2",
	.id_table       = cpufreq_interactive2_ids,
};
			   ...
			   const char *buf, size_t count)
{
	...
	if (boost_val) {
		trace_cpufreq_interactive2_boost("on");
		cpufreq_interactive2_boost();
	} else {
		trace_cpufreq_interactive2_unboost("off");
	}
				...
				const char *buf, size_t count)
{
	...
	trace_cpufreq_interactive2_boost("pulse");
	cpufreq_interactive2_boost();
	return count;
}
...
static struct attribute *interactive2_attributes[] = {
...
static struct attribute_group interactive2_attr_group = {
	.attrs = interactive2_attributes,
	.name = "interactive2",
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		rc = sysfs_create_group(cpufreq_global_kobject,
				&interactive2_attr_group);
		if (rc)
			return rc;

		rc = input_register_handler(&cpufreq_interactive2_input_handler);
		if (rc)
	...
	case CPUFREQ_GOV_STOP:
		...
		input_unregister_handler(&cpufreq_interactive2_input_handler);
...
static int cpufreq_interactive2_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_interactive2_idle_start();
		break;
	case IDLE_END:
		cpufreq_interactive2_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_interactive2_idle_nb = {
	.notifier_call = cpufreq_interactive2_idle_notifier,
};

static int __init cpufreq_interactive2_init(void)
{
	unsigned int i;
	struct cpufreq_interactive2_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_interactive2_timer;
		pcpu->cpu_timer.data = i;
		pcpu->cpu_tune_value = DEFAULT_TUNE;
	}

	up_task = kthread_create(cpufreq_interactive2_up_task, NULL,
				 "kinteractive2up");
	if (IS_ERR(up_task))
	...
	INIT_WORK(&freq_scale_down_work,
		  cpufreq_interactive2_freq_down);

	INIT_WORK(&tune_work,
		  cpufreq_interactive2_tune);
	...
	idle_notifier_register(&cpufreq_interactive2_idle_nb);
	INIT_WORK(&inputopen.inputopen_work, cpufreq_interactive2_input_open);
	return cpufreq_register_governor(&cpufreq_gov_interactive2);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_INTERACTIVE2
fs_initcall(cpufreq_interactive2_init);
#else
module_init(cpufreq_interactive2_init);
#endif

static void __exit cpufreq_interactive2_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_interactive2);
	kthread_stop(up_task);
	put_task_struct(up_task);
	destroy_workqueue(tune_wq);
}

module_exit(cpufreq_interactive2_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_DESCRIPTION("'cpufreq_interactive2' - A cpufreq governor for "
